...
export async function validateAll() {
  ...
  const unitOverrideMap = new Map();
  const gramsPerFields = [
    'grams_per_count',
    'grams_per_medium',
    'grams_per_large',
    'grams_per_piece',
  ...
  ingredientCatalogRows.forEach((row) => {
    ...
    gramsPerFields.forEach((field) => {
      const value = Number(row[field]);
      if (Number.isFinite(value) && value > 0) {
        if (!unitOverrideMap.has(row.ingredient_id)) {
          unitOverrideMap.set(row.ingredient_id, new Set());
        }
        unitOverrideMap.get(row.ingredient_id).add(field.replace('grams_per_', ''));
      }
    });
  });
  ...
  for (const dirEnt of recipeDirs) {
    ...
    for (const row of ingredientRows) {
      ...
      if (normalizedUnit && disallowedUnits.has(normalizedUnit) && produceHerbIds.has(row.ingredient_id)) {
        const allowedUnits = unitOverrideMap.get(row.ingredient_id) || new Set();
        if (!allowedUnits.has(normalizedUnit)) {
          unitLintWarnings.push(
            `${recipeId}: ${row.ingredient_id} uses "${row.unit}" (token ${token}). ` +
            'Prefer mass/volume units for produce/herbs or add grams_per_* in ingredient_catalog.csv.'
          );
        }
      }
    }
